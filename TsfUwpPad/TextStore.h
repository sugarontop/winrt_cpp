...
class CTextStore : public ITextStoreACP2
{
public:
    ...
    void OnCaret1Caret2ChangeFromEditor()
    {
        if (_pSink)
            _pSink->OnSelectionChange(); // Ž–Œã•ñ
    }

    void OnTextChangeFromEditor(LONG acpStart, LONG acpOldEnd, LONG acpNewEnd)
    {
        if (_pSink)
        {
            ...
            _pSink->OnTextChange(0, &textChange); // Ž–Œã•ñ
        }
    }
